...
define(function(require, exports, module) {
	...
	var ua = navigator.userAgent.toLowerCase(),
		...
		ieversion = isie ? ua.match(/msie ([\d.]+)/)[1] : null,
	...
	var charCount_init = function(content, tipmsg, options){
		...
		var data_charcount = $content.data('data-charcount'),
			tagname = ($content[0].tagName || '').toLowerCase(),
			timeoutid = 0;
		...
		var charCount = {
			...
			getAlready : function(){
				var text = _getval(),
					count =  options.half ? _half(text) : text.length;
